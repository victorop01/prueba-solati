...
export class Controller {
  ...
  async deleteTask(req: IRequest, res: Response, next: NextFunction) {
    const { id } = req.params;

    try {
      const deletedRows = await TaskModel().destroy({
        where: {
          id
        }
      });
      ...
      if(deletedRows){
        return res.status(200).json({ 
          success: true,
          message: `Se elimina correctamente la tarea con el id ${id}`,
        });
      } else {
        return res.status(404).json({
          message: `No se encuentra la tarea con el id ${id}`,
